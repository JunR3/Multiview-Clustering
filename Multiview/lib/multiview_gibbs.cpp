...
Rcpp::List run_gibbs_cpp(const Rcpp::List &data_views, int M, int burn_in,
  ...
  return Rcpp::List::create(Rcpp::Named("table_of") = saved_table_of,
...
void gibbs_sampler(int M, int burn_in, int thin) {
  ...
  for (auto &m : workspace_cache)
    m.reserve(64);

  for (int iter = 0; iter < M; ++iter) {

    if ((iter + 1) % 100 == 0) {
      Rcpp::Rcout << "Iteration " << (iter + 1) << " / " << M << std::endl;
    }
