...
double log_posterior_sigma_view(int v, double sigma_candidate) {
  ...
  if (sigma_candidate <= kEps || sigma_candidate >= 1.0 - kEps)
    return -INFINITY;
...
double log_posterior_given_tau(int v, double tau_candidate) {
  ...
  double logprior = a_tau * std::log(b_tau) - std::lgamma(a_tau) -
...
void update_hyperparameters() {
  ...
  for (int v = 0; v < d; ++v) {
    ...
    if (std::log(uniform01()) < log_posterior_sigma_view(v, sigma_prop) -
  ...
  if (std::log(uniform01()) < log_posterior_global_sigma(sg_prop) -
